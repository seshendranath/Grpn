...
SELECT    c.collection_id ,
         ...
         datediff(from_unixtime(unix_timestamp(cast(c.collection_date_key as string) ,'yyyyMMdd'), 'yyyy-MM-dd'),from_unixtime(unix_timestamp(cast(dl.actual_start_date_key as string) ,'yyyyMMdd'), 'yyyy-MM-dd')) AS coll_days_after_deal_feature ,
         datediff(from_unixtime(unix_timestamp(cast(c.collection_date_key as string) ,'yyyyMMdd'), 'yyyy-MM-dd'),from_unixtime(unix_timestamp(cast(dl.actual_end_date_key as string) ,'yyyyMMdd'), 'yyyy-MM-dd')) AS coll_days_after_deal_close ,
          CASE
          ...
          END , 0 ) +
          CASE
  WHEN c.collection_date_key <=  regexp_replace(date_add(from_unixtime(unix_timestamp(COALESCE( cast(dl.actual_end_date_key as string) , 30000101 ) ,'yyyyMMdd'), 'yyyy-MM-dd'), 60),'-','') THEN (
                              CASE
                                        WHEN c.payment_type = 'discount'
                                        AND       c.action = 'cancel' THEN COALESCE( c.amount , 0 )
                                        ELSE COALESCE(
...
WHEN c.collection_date_key <=  regexp_replace(date_add(from_unixtime(unix_timestamp(COALESCE( cast(dl.actual_end_date_key as string) , 30000101 ) ,'yyyyMMdd'), 'yyyy-MM-dd'), 60),'-','') THEN (
          ...
          CASE
                    WHEN (
                              ...
                              AND       c.action = 'cancel'
                              AND       (
                                                  c.collection_date_key <= COALESCE( cast(dl.actual_end_date_key as string) , 30000101 ) ) ) THEN COALESCE( c.amount , 0 )
                    ELSE 0.00
          END +
          CASE
                    WHEN (
                              ...
                              AND       (
                                                  c.collection_date_key <= COALESCE( cast(dl.actual_end_date_key as string) , 30000101 ) ) ) THEN
                              CASE
                                        WHEN (
                                                            c.payment_type = 'shipping' ) THEN 0.00
...
AND   c.collection_date_key <=  regexp_replace(date_add(from_unixtime(unix_timestamp(COALESCE( cast(dl.actual_end_date_key as string) , 30000101 ) ,'yyyyMMdd'), 'yyyy-MM-dd'), 60),'-','') ) THEN COALESCE( c.refund_amount , 0 )
                    ELSE 0.00
...
AND   c.collection_date_key <=  regexp_replace(date_add(from_unixtime(unix_timestamp(COALESCE( cast(dl.actual_end_date_key as string) , 30000101 ) ,'yyyyMMdd'), 'yyyy-MM-dd'), 60),'-','') ) THEN COALESCE( c.refund_amount , 0 )
  ...
  ( (CASE WHEN c.payment_type = 'discount' AND c.action = 'cancel' THEN COALESCE( c.amount , 0 )
                    ELSE ( CASE WHEN ( c.payment_type = 'shipping' ) THEN 0.00
                                ...
                                ELSE COALESCE( c.refund_amount , 0 )
                            END )
          END) - (CASE WHEN ( c.payment_type = 'discount' AND c.action = 'cancel' AND ( c.collection_date_key <= COALESCE( cast(dl.actual_end_date_key as string) , 30000101 ) ) ) THEN COALESCE( c.amount , 0 )
      ...
      AND ( c.collection_date_key <= COALESCE( cast(dl.actual_end_date_key as string) , 30000101 ) ) ) THEN
...
END)  )     AS refunds_less_resignation_amt ,
          ...
          CASE
                    WHEN (
                                        c.payment_type = 'creditcard'
AND   c.collection_date_key <=  regexp_replace(date_add(from_unixtime(unix_timestamp(COALESCE( cast(dl.actual_end_date_key as string) , 30000101 ) ,'yyyyMMdd'), 'yyyy-MM-dd'), 60),'-','')) THEN COALESCE(
                              CASE
                                        WHEN ord.unit_sell_price = 0 THEN 0.00
                                        WHEN c.payment_type = 'shipping' THEN 0.00
...
AND   c.collection_date_key <=  regexp_replace(date_add(from_unixtime(unix_timestamp(COALESCE( cast(dl.actual_end_date_key as string) , 30000101 ) ,'yyyyMMdd'), 'yyyy-MM-dd'), 60),'-','')) THEN COALESCE(
          ...
          CASE
  WHEN from_unixtime(unix_timestamp(cast(c.collection_date_key as string) ,'yyyyMMdd'), 'yyyy-MM-dd') > opt.expires_at THEN (
                              CASE
                                        WHEN c.payment_type = 'discount'
                                        AND       c.action = 'cancel' THEN COALESCE( c.amount , 0 )
                                        ELSE COALESCE( c.refund_amount , 0 )
  ...
  WHEN from_unixtime(unix_timestamp(cast(c.collection_date_key as string) ,'yyyyMMdd'), 'yyyy-MM-dd') > opt.expires_at THEN (
          ...
          CASE
  WHEN from_unixtime(unix_timestamp(cast(c.collection_date_key as string) ,'yyyyMMdd'), 'yyyy-MM-dd') BETWEEN (date_add(from_unixtime(unix_timestamp(cast(dl.actual_end_date_key as string) ,'yyyyMMdd'), 'yyyy-MM-dd'), 61)) AND opt.expires_at THEN (
                              CASE
                                        WHEN c.payment_type = 'discount'
                                        AND       c.action = 'cancel' THEN COALESCE( c.amount , 0 )
                                        ELSE COALESCE( c.refund_amount , 0 )

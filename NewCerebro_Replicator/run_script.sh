...
hdfs="hdfs://gdoop-namenode"
script_host="gdoop-replicator1.snc1"
backup_dir="user/grp_gdoop_edw_rep_prod"
user_account="svc_replicator_prod"
schema=$1
table=$2
inc_key=$3
inc_val=$4
...
echo "hadoop fs -ls ${hdfs}/${backup_dir}/${schema}.${table}/${inc_val_stripped}"
hadoop fs -ls ${hdfs}/${backup_dir}/${schema}.${table}/${inc_val_stripped}
ret_code=$?

if [ "$ret_code" -eq 0 ]; then
	echo " File Already Exist for the date ${inc_val} , Did you mean to re-load it?"
	echo " Assuming that a backfill is happening, and continuing, if this is not the intention, then contact aguyyala@"
	#job_status="failed"
fi
#else
#End of Comment
...
if [ $full_load -eq 0 ]; then
	echo " Picking the increment value as ${inc_val} "
		...
		echo "PYTHONPATH=$PYTHONPATH:/home/${user_account}/Replicator/ZombieRunner  PATH=/usr/local/bin/unixodbc:/opt/teradata/client/Current/tbuild/bin:/usr/local/bin:/usr/local/sbin:/usr/bin:/usr/sbin:/bin:/sbin:/home/${user_account}/bin zombie_runner run /home/${user_account}/Replicator/scripts/backup2cerebro/ --context=schema:${schema},base_table:${table},inc_key:\x22${inc_key}\x22,inc_date:${inc_val}"
        	output=` PYTHONPATH=$PYTHONPATH:/home/${user_account}/Replicator/ZombieRunner  PATH=/usr/local/bin/unixodbc:/opt/teradata/client/Current/tbuild/bin:/usr/local/bin:/usr/local/sbin:/usr/bin:/usr/sbin:/bin:/sbin:/home/${user_account}/bin zombie_runner run /home/${user_account}/Replicator/scripts/backup2cerebro/ --context=schema:${schema},base_table:${table},inc_key:\"${inc_key}\",inc_date:${inc_val}`

else
       echo "No inc_val and inc_key  was passed, Considering this table as a full refresh and defauling to today's date "
       inc_val=`date +%Y%m%d`
        echo "PYTHONPATH=$PYTHONPATH:/home/${user_account}/Replicator/ZombieRunner  PATH=/usr/local/bin/unixodbc:/opt/teradata/client/Current/tbuild/bin:/usr/local/bin:/usr/local/sbin:/usr/bin:/usr/sbin:/bin:/sbin:/home/${user_account}/bin zombie_runner run /home/${user_account}/Replicator/scripts/backup2cerebro/ --context=schema:${schema},base_table:${table}"

        output=` PYTHONPATH=$PYTHONPATH:/home/${user_account}/Replicator/ZombieRunner  PATH=/usr/local/bin/unixodbc:/opt/teradata/client/Current/tbuild/bin:/usr/local/bin:/usr/local/sbin:/usr/bin:/usr/sbin:/bin:/sbin:/home/${user_account}/bin zombie_runner run /home/${user_account}/Replicator/scripts/backup2cerebro/ --context=schema:${schema},base_table:${table} `
...
if [ "${td_count}" -eq "${hive_count_trimmed}" ]; then
	...
	echo " Counts have not matched, exiting, and the script will fail "
        job_status="failed"
        echo "Teradata Count : ${td_count} Hive Count : ${hive_count_trimmed} schema: ${schema} table: ${table} inc_key: ${inc_key} inc_val: ${inc_val}" | mailx -s "MISMATCH in GDOOP COPY schema: ${schema} table: ${table} inc_key: ${inc_key} inc_val: ${inc_val} Teradata Count : ${td_count} Hive Count : ${hive_count_trimmed} " -r aguyyala@groupon.com aguyyala@groupon.com

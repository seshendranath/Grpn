...
"""
Usage:
  RunReplicator.py (import | export) --table=<schema>.<table> [--source=<db>/<username>.<password>]
                   ...
                   [--method=<method>] [--HADOOP_CLASSPATH=<$HADOOP_CLASSPATH>] [--TDCH_JAR=<TDCHjarpath>] [--LIB_JARS=<HiveLibJarsPath>]
                   [--hive-only] [-v|-vv]
...
def get_prev_size():
    size = 0
    cmd = ["hadoop fs -du %s | tail -1 | awk '{print $1}'" % get('base_hdfs')]
    logging.info("Running "+" ".join(cmd))
...
def create_exec_copy_script():
    ...
    fo.write("-queryband 'TableName=%s.%s;RefreshType=%s;SplitBy=%s;Size=%s;NumMappers=%s;' " % (get('Tschema'), get('Ttable'), refresh, get('method'), args['size'], nummaps))

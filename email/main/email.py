...
def run_core_process():
    ...
    qry = """
            ...
            WHERE event='{open}'
            GROUP BY 1,2,3
          """.format(click=get_email('click'), open=get_email('open'))
    ...
    qry = """
            SELECT
                 ...
                 ,MIN(CASE WHEN clickDestination LIKE '%unsub%' THEN event_date ELSE '9999-12-31' END) AS unsub_date
                 ,SUM(CASE WHEN clickDestination LIKE '%unsub%' THEN 1 ELSE 0 END) AS unsub_click_cnt
            FROM {click}_table
            WHERE event='{click}'
            GROUP BY 1,2,3
    ...
    final_qry = """
            SELECT
                ...
                ,c.unsub_click_cnt AS unsub_click_cnt
                ...
                ,CASE WHEN s.event_date = c.unsub_date THEN 1 ELSE 0 END AS sd_unsub_cnt
                ...
                ,CASE WHEN datediff(c.unsub_date, s.event_date) BETWEEN 0 AND 2 THEN 1 ELSE 0 END AS d3_unsub_cnt
                ,CASE WHEN datediff(o.event_date, s.event_date) BETWEEN 0 AND 3 THEN 1 ELSE 0 END AS d4_open_cnt
                ,CASE WHEN datediff(c.event_date, s.event_date) BETWEEN 0 AND 3 THEN 1 ELSE 0 END AS d4_click_cnt
                ,CASE WHEN datediff(c.unsub_date, s.event_date) BETWEEN 0 AND 3 THEN 1 ELSE 0 END AS d4_unsub_cnt
                ...
                ,CASE WHEN datediff(c.unsub_date, s.event_date) BETWEEN 0 AND 6 THEN 1 ELSE 0 END AS d7_unsub_cnt
...
def kickoff():
    ...
    Actual KickOff
    """
    logging.info('Kicked Off')

    get_event_dates_to_process()

    if get('--assert_only'):
        logging.info('Running PRE and POST ASSERTIONS ONLY')
        create_data_frames_to_be_read()
        pre_assertion_checks()
        post_assertion_checks()
        return

    if get('--pre_assert_only'):
        logging.info('Running PRE ASSERTIONS ONLY')
        create_data_frames_to_be_read()
        pre_assertion_checks()
        return

    if get('--post_assert_only'):
        logging.info('Running POST ASSERTIONS ONLY')
        post_assertion_checks()
        return
